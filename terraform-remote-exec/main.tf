...
# it is used for executing some shell command on our remote machine be it azure, aws or gcp
...
resource "aws_instance" "jenkins-controller" {
  ...
  connection {
    ...
    user        = "ec2-user"
    private_key = file("~/Downloads/devops/terraform-test/terraform-remote-exec/keypair.pem")
    timeout     = "4m"
  ...
  provisioner "remote-exec" {
    inline = [ 
        "touch hello.txt",
        "echo i love remote provisioner >> hello.txt",
        "sudo yum update -y"
     ]
  }
